...
//
// It flies a straight climb eastward over rising ground, which is exactly the case Phase 0's
// claim is about: the altitude goes up, the ground goes up faster, and the AGL must FALL.
...
Bun.listen({
  ...
  socket: {
    ...
    open(socket) {
      console.log('client connected — flying east, climbing');
      let t = 12 * 3600, lat = 47.0, lon = 8.0, alt = 1500;
      const timer = setInterval(() => {
        t += 1; lon += 0.0002; alt += 1.5;   // ~15 m/s east, +1.5 m/s
        ...
        socket.write(
          cs(`$GPGGA,${hms},${dm(lat)},N,0${dm(lon)},E,1,08,1.0,${alt.toFixed(1)},M,47.0,M,,`) + '\r\n'
          + cs(`$GPRMC,${hms},A,${dm(lat)},N,0${dm(lon)},E,30.0,090.0,110726,,,A`) + '\r\n'
